...
if size(subNiiFile,1) == 0 % there is no nii file in subject subnum
    flag = -1;
    errorsub = subNum;
    error(['there is no .nii or .nii.gz file in' sublist(1).name])
end
...
for subNum = 1:Nsub
    ...
    [volData, volInfo] = NDN_Read(dfile(1).name);
