...
软件主界面包括三部分，分别为数据提取模块、方法选择模块和聚类与绘图模块，如图1所示。
...
1) 基于感兴趣区的分析方法（ROI Methods）：这种方法聚焦于预先定义的感兴趣区域（ROI）进行分析。ROI 可以是某个特定的脑区或其他研究者感兴趣的区域。ROI方法通常用于简化分析，减少多重比较的问题，并提高统计效能。应用场景：常用于假设驱动的研究，例如研究某个脑区在特定任务中的激活情况。

2) 基于灰质体素的分析方法，这种方法在整个大脑或特定的脑结构中，以体素为单位（通常是灰质区域）进行全脑范围的分析。这种方法通常是探索性的，没有预先定义的感兴趣区域，而是通过对所有体素进行分析，找出与实验条件相关的区域。常用于无假设或数据驱动的研究，如全脑分析，用于检测大脑不同区域的体积变化或激活模式。

 

在聚类与绘图模块中，你可以通过Best K这一功能求出最佳的K值，或者自己设定一个K值，让本工具将动态的脑网络分析方法的结果进行一个聚类，最终得出K个聚类中心也即是状态，并将这K个状态在界面上可视化，同时将这一组被试的状态转移矩阵绘制出来。
...
首先，我们数据的组织形式应该如下图所示，当前的目录下，有3个被试，分别为sub-UTS01、02、03，对于每一个被试对应的fMRI文件存放到一个单独的文件夹下，并且每个文件夹命名要有一定规律，例如都是以字符串‘sub’开头，这样做的目的是为了排除其他无关的文件和文件夹。
...
第五，由于ROI mask种类繁多，为了对后续分析造成不必要的混淆，所以输入一个可以辨别的标签Tag，让后人知道该ROI的时间序列是在哪一个ROI mask下得到的。

  ![1732705556723](assets/README_pics/1732705556723.png)

当按照上述的正确输入之后，点击按钮Run即可运行，运行成功会如图4底部所示，告知你结果已经保存在你选定的目标文件夹下面。每一个被试的ROI时间序列会有一个对于的mat文件，这是一个二维矩阵数据，行数代表了时间点的个数，列数代表了ROI的个数，结果如图5所示。
...
基于感兴趣区的分析方法本软件实现了10种动态分析方法(动态条件相关DCC, 基于滑动窗口的动态功能链接SWFC, 可变最小二乘法FLS, 广义线性卡尔曼滤波器GLKF, 时间导数乘积MTD及其5种对应的增强版本,被试间动态条件相关ISDCC,基于滑动窗口的被试间动态功能链接ISSWFC, 被试间可变最小二乘法ISFLS,被试间广义线性卡尔曼滤波器ISGLKF, 被试间时间导数乘积ISMTD)和1种静态分析方法(静态功能链接SFC)， 共11种方法基于感兴趣区的分析方法。
...
1) DCC界面的参数包括了TR，TR 是指 Repetition Time，即重复时间，连续两次脉冲之间的时间间隔，取值范围需大于0(下同)。
...
regressLOO，是在LOO的基础上，对每个被试的源数据(维度为nT * nROI）与 LOO_Mean(维度为nT * nROI）	时间序列进行回归分析，通过去均值和添加截距项来调整数据，接	着使用回归计算得到残差，并从原始数据中减去这些残差以去除协	变量的影响。最后，处理无效数据并恢复数据的均值，返回去除协变量影响后的时间序列数据(维度为nT * nROI）。这样的数据可以大约认为回归掉了一些无关的信号(如被试的自发信号、核磁共振设备的系统噪声、被试的生理噪声)，只剩下了自然刺激引发的大脑信号。

通过被试间的ISA分析，可以一定程度提升结果的在时间和空间上的一致性。

最后， 方法运行完成之后的结果都会保存在指定的位置下，如图7所示。每一个人的结果都会对应一个以“.mat”结尾的文件。对于动态分析方法运行的结果，会额外多出一个以“_all.mat”结尾的文件里面包含了这一组被试所有的结果还有一些参数信息，是作为第三步聚类和绘图模块的输入。

![1732705671810](assets/README_pics/1732705671810.png)

#### **3.2.2 基于灰质体素的分析方法**

基于灰质体素的分析方法本软件实现了3种，分别为共激活模式分析CAP、被试间共激活模式分析ISCAP、被试间相关ISC。

首先，介绍以下输入文件的组织形式。我们数据的组织形式应该如下图所示，当前的目录下，有3个被试，分别为sub-UTS01、02、03，对于每一个被试对应的fMRI文件和相应的头动文件存放到一个单独的文件夹下，头动文件必须以“.txt”结尾。并且每个文件夹命名要有一定规律，例如都是以字符串‘sub’开头，这样做的目的是为了排除其他无关的文件和文件夹。如果你想运行的是被试间相关ISC方法的话，是在文件夹下面放置无需头动文件的。如果你想运行的是（被试间）共激活模式分析(IS)CAP方法的话,是需要添加头动文件的，没有添加头动文件也可以运行，软件会默认为0头动。有一点需要注意的是这个头动文件的时间长度应该和fMRI文件的时间长度保持一致，否则会报错。
...
当按照上述的正确输入之后，点击按钮Run即可运行，运行成功会如图9底部所示，告知你结果已经保存在你选定的目标文件夹下面。每一个被试的ISC结果都会对应有两个 3D NII文件，一个是ISC算出来的直接结果，另外一个是在此基础上做了一次Fisher's Z 变换。运行之后结果如图10所示，如果你在MATLAB添加了[BrainNet Viewer](https://www.nitrc.org/projects/bnv/)工具包，那么每一个3D文件都会生成一个相应的TIF图片。
...
（4）接下来就是在页面输入合适的ISCAP参数。参数runName可以用来标识当前这组被试的数据。Tmot作为一个头动阈值，软件会根据Tmot的数值和头动文件的内容用来筛出掉fMRI那些头动较大时的帧。K代表了聚类个数，一般K的输入范围在1-12。TR 是指 Repetition Time，即重复时间，连续两次脉冲之间的时间间隔。pN, pP代表的含义分别指定应保留用于聚类的正值和负值体素的比例（其余体素则设置为零）。pN, pP的取值范围是 [1，100]。
